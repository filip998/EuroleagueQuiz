...
def _ingest_player(
    ...
    memberships = _merge_local_euroleague_memberships(
        memberships,
...
def _local_euroleague_memberships(session: Session, player: Player) -> list[CareerMembership]:
    rows = (
    ...
    def flush_current() -> None:
        ...
        memberships.append(
            CareerMembership(
                ...
                source_team_key=_local_team_key(current_team),
                source_team_label=current_team.name,
                source_team_url=None,
                source_row_key=f"local:euroleague:{player.id}:{current_team.id}:{start_year}",
                local_team_id=current_team.id,
                raw_start=str(start_year),
                raw_end=None if is_current else str(end_year),
                start_year=start_year,
                end_year=None if is_current else end_year,
                is_current=is_current,
                order_hint=50_000 + start_year,
            )
        )

    for _, team, season in rows:
...
def _merge_local_euroleague_memberships(
    wikipedia_memberships: list[CareerMembership],
    local_memberships: list[CareerMembership],
) -> list[CareerMembership]:
    merged = list(wikipedia_memberships)
...
def _mergeable_same_team_index(
    memberships: list[CareerMembership],
    local: CareerMembership,
) -> int | None:
    for index, membership in enumerate(memberships):
        if membership.source_team_key != local.source_team_key:
            continue
        if _ranges_overlap_or_are_adjacent(
...
def _ranges_overlap_or_are_adjacent(
    ...
    if left_start is None or right_start is None:
        return False
    effective_left_end = left_end if left_end is not None else right_start
    effective_right_end = right_end if right_end is not None else left_start
    return left_start <= effective_right_end + 1 and right_start <= effective_left_end + 1


def _merge_memberships(existing: CareerMembership, local: CareerMembership) -> CareerMembership:
    start_year = min(year for year in (existing.start_year, local.start_year) if year is not None)
    if existing.end_year is None or (
        local.end_year is None
        and local.start_year is not None
        and existing.end_year is not None
        and local.start_year > existing.end_year
    ):
        end_year = None
    elif local.end_year is None:
        end_year = existing.end_year
    else:
        end_year = max(existing.end_year, local.end_year)
    return CareerMembership(
        source_name=existing.source_name,
        source_player_key=existing.source_player_key,
        source_team_key=existing.source_team_key,
        source_team_label=existing.source_team_label,
        source_team_url=existing.source_team_url,
        source_row_key=existing.source_row_key,
        local_team_id=existing.local_team_id or local.local_team_id,
        raw_start=str(start_year),
        raw_end=str(end_year) if end_year is not None else None,
        start_year=start_year,
        end_year=end_year,
