...
import { getRosterGame, submitRosterGuess, offerEndRound, respondEndRound, connectRosterWebSocket, autocompleteRosterPlayer } from "./api";

const POSITION_ORDER = { "Center": 0, "Forward-Center": 1, "Forward": 2, "Guard-Forward": 3, "Guard": 4 };
...
export default function RosterGuessBoard({ initialState, onNewGame, onHome, onlineInfo }) {
    ...
    function connect() {
      ...
      ws = connectRosterWebSocket(game.id, myPlayer, (data) => {
        if (data.error) { setError(data.error); } else {
          ...
          if (result && ["round_won","round_complete","match_won"].includes(result)) startRoundTransition(result);
  ...
  useEffect(() => {
    ...
    if (timeLeft <= 0) { setGame(p => ({ ...p, current_player: p.current_player === 1 ? 2 : 1 })); setLastResult("time_expired"); return; }
  ...
  function startRoundTransition(result) { setRoundTransition({ countdown: 3, result }); setLastResult(result); setSearchQuery(""); setSearchResults([]); }
  ...
  async function handlePlayerSelect(player) {
    ...
    try {
      if (isOnline && wsRef.current?.readyState === WebSocket.OPEN) {
        ...
        setGame(res.game);
        if (["round_won","round_complete","match_won"].includes(res.result)) startRoundTransition(res.result);
  ...
  async function handleRespondEnd(accept) {
    ...
    try {
      if (isOnline && wsRef.current?.readyState === WebSocket.OPEN) wsRef.current.send(JSON.stringify({ action: "respond_end", accept }));
        ...
        if (accept && ["round_won","round_complete","match_won"].includes(r.result)) startRoundTransition(r.result);
  ...
  const resultMessages = { correct: "\u2705 Correct!", incorrect: "\u274c Wrong player.", round_won: "\ud83c\udfc6 Round over!", round_complete: "\ud83c\udfc6 Round over!", match_won: "\ud83c\udf89 Match won!", end_offered: "\ud83e\udd1d End offered.", end_accepted: "\ud83e\udd1d Round ended!", end_declined: "Declined.", time_expired: "\u23f0 Time\u2019s up!" };
  ...
  const canGuess = game.status === "active" && !inTransition && !game.pending_end && isMyTurn;

  if (game.status === "finished" && !inTransition) {
    ...
    <div className="h-screen flex flex-col overflow-hidden">
      ...
      <div className="bg-white border-b border-elq-border flex-shrink-0">
        <div className="max-w-5xl mx-auto px-3 py-2 flex items-center justify-between">
          ...
          <span className="text-[11px] text-elq-muted">Rd {game.round_number} &middot; First to {game.target_wins}</span>
        </div>
      </div>
      ...
      <div className="bg-white border-b border-elq-border flex-shrink-0">
        <div className="max-w-5xl mx-auto px-3 py-2 flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 flex-1 min-w-0">
            <span className={`font-semibold text-sm truncate ${game.current_player === 1 && game.status === "active" ? "text-elq-player1" : "text-elq-muted"}`}>{game.player1_name}</span>
            <span className="text-xl font-bold text-elq-dark">{game.player1_score}</span>
            {game.current_player === 1 && game.status === "active" && <span className="w-1.5 h-1.5 rounded-full bg-elq-player1 animate-pulse flex-shrink-0" />}
          </div>
          <div className="text-center flex-shrink-0 px-3">
            {game.turn_seconds && game.status === "active" && timeLeft !== null ? (<span className={`text-lg font-bold font-mono tabular-nums ${timeLeft <= 5 ? "animate-timer-critical" : "text-elq-dark"}`}>{timeLeft}<span className="text-[10px] text-elq-muted font-normal">s</span></span>) : (<span className="text-xs text-elq-muted">{currentPlayerName}&apos;s turn</span>)}
          </div>
          <div className="flex items-center gap-2 flex-1 min-w-0 justify-end">
            {game.current_player === 2 && game.status === "active" && <span className="w-1.5 h-1.5 rounded-full bg-elq-player2 animate-pulse flex-shrink-0" />}
            <span className="text-xl font-bold text-elq-dark">{game.player2_score}</span>
            <span className={`font-semibold text-sm truncate ${game.current_player === 2 && game.status === "active" ? "text-elq-player2" : "text-elq-muted"}`}>{game.player2_name}</span>
          </div>
        </div>
      </div>
      <div className="bg-elq-dark flex-shrink-0">
        <div className="max-w-5xl mx-auto px-3 py-2.5 flex items-center justify-between gap-3">
          <div className="flex items-baseline gap-2 min-w-0">
            <span className="font-display text-xl sm:text-2xl text-white tracking-wide truncate">{round.team_name}</span>
            <span className="text-elq-orange font-semibold text-sm whitespace-nowrap">{round.season_year}/{String(round.season_year + 1).slice(2)}</span>
          </div>
          <div className="flex items-center gap-3 text-[11px] text-white/60 whitespace-nowrap">
            ...
            {(round.player1_correct > 0 || round.player2_correct > 0) && (<span><span className="text-blue-300">{round.player1_correct}</span> &ndash; <span className="text-red-300">{round.player2_correct}</span></span>)}
          </div>
        </div>
      </div>
      {canGuess && (
      ...
      {(lastResult || error) && (<div className="flex-shrink-0 px-3 pt-2 max-w-5xl mx-auto w-full">{lastResult && (<div className={`px-3 py-1.5 rounded-lg text-center text-xs font-medium animate-slide-down ${["round_won","match_won","round_complete"].includes(lastResult) ? "bg-elq-orange/10 text-elq-orange" : lastResult === "correct" ? "bg-emerald-50 text-emerald-700" : lastResult === "incorrect" || lastResult === "time_expired" ? "bg-red-50 text-red-600" : "bg-amber-50 text-amber-700"}`}>{resultMessages[lastResult] || lastResult}{inTransition && <span className="ml-2 font-bold">Next in {roundTransition.countdown}...</span>}</div>)}{error && <div className="px-3 py-1.5 rounded-lg bg-red-50 text-red-600 text-xs text-center">{error}</div>}</div>)}
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-5xl mx-auto px-3 py-2">
          <table className="w-full text-sm border-collapse">
            <thead><tr className="text-[11px] uppercase tracking-wider text-elq-muted border-b-2 border-elq-border"><th className="text-left py-2 pr-2 w-10 font-semibold">#</th><th className="text-left py-2 pr-2 w-12 font-semibold">Pos</th><th className="text-left py-2 pr-2 font-semibold">Nat</th><th className="text-left py-2 pr-2 w-14 font-semibold hidden sm:table-cell">Ht</th><th className="text-left py-2 font-semibold">Player</th></tr></thead>
            <tbody>
              {sortedSlots.map((slot, i) => {
                ...
                return (
                  <tr key={slot.id} className={`border-b border-elq-border/50 transition-colors ${p1 ? "bg-blue-50/60" : p2 ? "bg-red-50/60" : i % 2 === 0 ? "bg-white" : "bg-slate-50/50"}`}>
                    <td className="py-1.5 pr-2"><span className={`inline-flex items-center justify-center w-7 h-7 rounded-md text-[11px] font-bold font-mono ${p1 ? "bg-elq-player1/10 text-elq-player1" : p2 ? "bg-elq-player2/10 text-elq-player2" : "bg-slate-100 text-slate-500"}`}>{slot.jersey_number || "?"}</span></td>
                    <td className="py-1.5 pr-2 text-xs text-elq-muted font-medium">{posAbbr(slot.position)}</td>
                    <td className="py-1.5 pr-2 text-xs text-elq-muted">{slot.nationality || "\u2014"}</td>
                    <td className="py-1.5 pr-2 text-xs text-elq-muted hidden sm:table-cell">{slot.height_cm || "\u2014"}</td>
                    <td className="py-1.5">{guessed ? (<span className={`font-semibold text-sm animate-cell-claim ${p1 ? "text-elq-player1" : "text-elq-player2"}`}>{slot.player_name}</span>) : (<span className="text-slate-300 text-sm">???</span>)}</td>
                  </tr>
                );
              })}
            </tbody>
