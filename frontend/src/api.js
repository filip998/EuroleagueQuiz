...
export function submitRosterGuess(gameId, playerId, options = {}) {
  const query = options.playerNumber != null ? `?player=${options.playerNumber}` : "";
  const body = { player_id: playerId };
  if (options.roundNumber != null) body.round_number = options.roundNumber;
  return actionRequest("POST", `/quiz/roster-guess/games/${gameId}/guess${query}`, body);
}
