...
export default function GuessTheListRaceBoard({ initialState, onlineInfo, onNewGame, onHome }) {
  ...
  useEffect(() => {
    const timer = setTimeout(async () => {
      ...
      try {
        const data = await autocompleteGuessTheListPlayer(searchQuery);
  ...
  async function handlePlayerSelect(player) {
    ...
    try {
      ...
      handleRealtimeState(await submitGuessTheList(game.id, player.player_id, roundNumber));
    } catch (error) {
  ...
  async function refreshGame() {
    if (!game?.id) return;
    try {
      setGame(await getGuessTheListGame(game.id));
    } catch {
  ...
  async function handleResign() {
    setResigning(true);
    try {
      handleRealtimeState(await resignGuessTheListRaceGame(game.id, playerNumber));
    } catch (error) {
      setMessage(error.message);
    } finally {
      setResigning(false);
  ...
  async function handleQuickCancel() {
    ...
    setCancelling(true);
    try {
      await cancelGuessTheListRaceQuickMatch({ preset: game.preset, game_id: game.id });
      clearOnlineInfo(game.id);
      forgetQuickMatchSeat(guessTheListRaceSeatKey(game.id));
      forgetQuickMatchSeat(legacyGuessTheListRaceSeatKey(game.id));
      onNewGame();
    } catch {
      setCancelling(false);
    }
    ...
    if (game.is_public && game.preset) {
      return (
        <QuickMatchSearchingLobby
          ...
          usePools={useGuessTheListRaceQuickMatchPools}
          getPresetLabel={guessTheListRacePresetLabel}
          title="SEARCHING GUESS THE LIST RACE..."
        />
      );
    }
    return (
...
function RaceScoreboard({ game, round, timerRemaining, playerNumber }) {
  return (
    <OnlineScoreboard
      ariaLabel="Guess the List Race multiplayer scoreboard"
      players={[
        {
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <div className="bg-white border-b border-elq-border">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <span className="font-display text-base tracking-wide text-elq-dark">GUESS THE LIST RACE</span>
          <span />
        </div>
      </div>
